...
class FeatureParser extends Filter {
  ...
  processString(content, relativePath) {
    ...
    let testFolder = pathParts[3];
    let nestedFolderParts = pathParts.slice(4, -1);
    let stepsPath = [basePath, 'yadda', testFolder, 'steps', ...nestedFolderParts].join('/');
  ...
  getDestFilePath(relativePath) {
    ...
    if (ext === '.feature' || ext === '.spec' || ext === '.specification') {
      return relativePath.replace(ext, '-yadda-test.js');
    }
    return null;
  }
}
